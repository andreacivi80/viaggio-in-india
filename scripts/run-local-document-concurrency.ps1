...
"@
  ...
  if ($Suite -in @("ui-session-history", "ui-location-permissions", "ui-microphone-permissions", "ui-password-access")) {
    $uiSetupSql = @"
INSERT INTO profiles(id,name,surname,role,created_at) VALUES
...
"@
  ...
  foreach ($selectedSuite in $selectedSuites) {
    ...
    $previousErrorActionPreference = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    if ($selectedSuite -in @("ui-session-history", "ui-location-permissions", "ui-microphone-permissions", "ui-password-access")) {
