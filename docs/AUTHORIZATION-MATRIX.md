...
La precedente modalità client `Vista pubblica/Vista gruppo` è stata rimossa integralmente perché non rappresentava uno stato autorizzativo server e non era più raggiungibile dall’interfaccia. Non esiste quindi un interruttore capace di nascondere localmente una sessione mantenendo comportamenti contraddittori: Pubblico significa assenza di sessione personale verificata; Viaggiatore e Coordinatore derivano esclusivamente da `auth/session`. `ui-coordinator-grid-access.spec.mjs`, ripetuto due volte su Galaxy S20 FE, confronta un browser Pubblico, uno con sola password, un Viaggiatore e un Coordinatore; verifica assenza dei comandi documenti, posizione e amministrazione nei primi stati, identità coordinatore invariata durante la navigazione e conferma finale server della sessione.

La facoltà di creare un invito non produce né trasferisce una sessione: `auth/invites` restituisce soltanto identificatore, token monouso, scadenza e profilo destinatario. La matrice dinamica prova che il token appena creato riceve `401` da `/api/private`, che la risposta non contiene documenti o token di sessione e che profilo e ruolo del Coordinatore rimangono identici prima e dopo la creazione. L’eventuale lettura documenti del Coordinatore dipende quindi dal suo ruolo server già autorizzato, non dall’atto di creare l’invito.

La password comune non trasforma un commentatore in Viaggiatore. `ui-password-access.spec.mjs` verifica tramite tocco che, dopo la password, il dispositivo rimane Pubblico; inserendo un nome viene creata una distinta sessione Ospite vincolata al telefono e il commento riceve `201`. Quando la stessa persona completa poi la registrazione con consenso, le credenziali Ospite vengono rimosse e resta soltanto la nuova sessione personale. Il cambio di scheda chiude inoltre il pannello di accesso, impedendo che intercetti i tocchi della Bacheca.
...
La proroga della conservazione è a doppio passaggio: `POST /api/retention-extension` accetta soltanto il profilo della sessione richiedente; `PUT /api/retention-extension/:profileId` accetta soltanto la coordinatrice e decide esclusivamente una richiesta ancora pendente. Il pubblico, i familiari e gli altri viaggiatori non vedono la richiesta privata.
