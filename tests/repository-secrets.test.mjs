...
test("password e file token non sono tracciati nel repository", async () => {
  ...
  const knownGroupCodes = [["india", "26"].join(""), ["Thailandia", "2026"].join("")];
  ...
  for (const path of scan) {
    ...
    for (const knownGroupCode of knownGroupCodes)
      assert.ok(!source.toLowerCase().includes(knownGroupCode.toLowerCase()), `${path}: password comune scritta nel file`);
  }
});
